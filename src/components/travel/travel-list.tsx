...
import { Plus, Search, Trash2, CalendarPlus, Check, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
            ...
            <tbody>
              {filtered.map((item) => {
                ...
                return (
                  ...
                  >
                    {/* 액션: 가본곳 / 캘린더추가 / 삭제 */}
                    <td className="px-2 py-2.5 border-r whitespace-nowrap">
                      <div className="flex items-center gap-0.5">
                        <button
                          type="button"
                          title="가본 곳 토글"
                          className={`rounded p-1 hover:bg-accent ${item.visited ? "text-green-600" : "text-muted-foreground hover:text-green-600"}`}
                          onClick={(e) => { e.stopPropagation(); toggleVisited(item.id, item.visited); }}
                        >
                        ...
                        <button
                          type="button"
                          title="캘린더에 추가"
                          className="rounded p-1 text-muted-foreground hover:text-blue-600 hover:bg-accent"
                          onClick={(e) => { e.stopPropagation(); setCalendarItem(item); }}
                        >
                    ...
                    <td className="px-3 py-2.5 border-r overflow-hidden">
                      <div className="flex items-center gap-2">
                        <span className="inline-block h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: item.color || color }} />
